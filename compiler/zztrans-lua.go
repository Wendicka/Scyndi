...
mkl.Version("Scyndi Programming Language - zztrans-lua.go","19.02.08")
	...
	tmw.operators = NewDefOperators(); // We need to have the basic. As this is not a pointer assignment, I can just modify all this :)
	tmw.operators[  "="]="=="
	tmw.operators[ "=="]="=="
	tmw.operators[ "~="]="~="
	tmw.operators[ "!="]="~="
	...
	tmw.TransTypes = func(src *tsource) string {
		ret:=""
		for vname,vdata:=range src.identifiers {
	...
	tmw.TransTypeDefinition = func(t *tsource,dtype *tidentifier,did *tidentifier) string{
	...
	tmw.TransTypeKill = func(t *tsource,dtype *tidentifier,did *tidentifier) string{
		if dtype.imported { throw("KILL not possible for imported types") }
		return did.translateto+" = nil"
	}
